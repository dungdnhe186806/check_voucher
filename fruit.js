javascript:(()=>{/* Shopee Farm Helper v2.3 - remove Goto button, keep draggable */
...
const GAME_URL="https://games.shopee.vn/farm";
...
function inject(){if(q("#sf_panel"))return;const st=document.createElement("style");st.textContent=STYLE;document.head.appendChild(st);const w=document.createElement("div");w.id="sf_panel";w.innerHTML=`
...
bind();}
function makeDraggable(panel,handle){let offsetX=0,offsetY=0,isDown=false;handle.addEventListener("mousedown",e=>{isDown=true;offsetX=e.clientX-panel.offsetLeft;offsetY=e.clientY-panel.offsetTop;document.body.style.userSelect="none"});document.addEventListener("mouseup",()=>{isDown=false;document.body.style.userSelect=""});document.addEventListener("mousemove",e=>{if(!isDown)return;panel.style.left=(e.clientX-offsetX)+"px";panel.style.top=(e.clientY-offsetY)+"px";panel.style.right="auto"})}
...
function levelInfo(crop){const state=(crop?.state??-1),expNow=(crop?.exp??0);const lc=crop?.meta?.config?.levelConfig||{};const lv=Object.entries(lc).filter(([k])=>!isNaN(k)&&parseInt(k)<100).map(([k,v])=>[parseInt(k),v.exp]).sort((a,b)=>a[0]-b[0]);const before=lv.filter(([l])=>l<state).reduce((s,[_ ,e])=>s+e,0);const tot=before+expNow;const max=lv.reduce((s,[_ ,e])=>s+e,0);const need=(lv.find(([l])=>l===state)||[0,0])[1];return {state,expNow,before,tot,max,need}}
async function callApi(endpoint,method="GET",payload=null){const opts={method,credentials:"include",headers:{}};if(method!=="GET"&&payload!=null){opts.headers["content-type"]="application/json;charset=UTF-8";opts.body=JSON.stringify(payload)}try{const r=await fetch(API_BASE+endpoint,opts);if(!r.ok)return{error:"HTTP "+r.status,raw:await r.text()};try{return await r.json()}catch(e){return{error:"JSON parse error",raw:await r.text()}}}catch(e){return{error:e.message}}}
async function ensureGameOpen(){if(location.host.includes("games.shopee.vn")){if(!hasShownFirstMessage){log("📌 Bookmarklet sẵn sàng — đang dùng cookie trình duyệt.");hasShownFirstMessage=true;}return true}else{const w=window.open(GAME_URL,"_blank");if(!w){alert("⚠️ Trình duyệt chặn mở tab. Hãy mở https://games.shopee.vn/farm rồi chạy lại bookmarklet.");return false}try{w.focus()}catch{}log("⚡ Đã mở tab game. Vui lòng chạy lại bookmarklet trên tab đó.");return false}}
...
async function buyProp(id){const prop=lastShopProps.find(p=>p.propMetaId==id);const r=await callApi("/prop/buy/v2","POST",{propMetaId:parseInt(id)});if(r?.code===0){log(`✅ Đã mua: ${prop?.name||"Bình"} (${prop?.parameter||"?"} nước) - ${prop?.price||"?"} xu`);await renderShop();}else log("❌ Mua lỗi:",r?.msg||JSON.stringify(r))}
async function waterByItem(id){const r=await callApi("/orchard/crop/use_bottled_water","POST",{cropId:0,propItemId:id});if(r?.code===0){log("Tưới thành công.");await renderStatus();await renderBag();}else log("Tưới lỗi:",r?.msg||JSON.stringify(r))}
async function harvest(){const ctx=await callApi("/orchard/context/get?skipGuidance=1&pre=1");if(ctx?.error)return log("Lỗi harvest:",ctx.error);const crop=ctx?.data?.crops?.[0];if(!crop)return log("Không có cây.");const r=await callApi("/orchard/crop/harvest","POST",{cropId:crop.id,metaId:crop.metaId});if(r?.code===0){const items=(r?.data?.reward?.rewardItems||[]).map(it=>`${it?.itemExtraData?.luckyDrawAwardName||it?.meta?.name||"Vật"} x${it?.itemExtraData?.gameItemInfo?.gameItem?.quantity||1}`).join(", ");log("Thu hoạch thành công:\n"+items);await renderStatus();}else log("Thu hoạch lỗi:",r?.msg||JSON.stringify(r))}
...
(async()=>{inject();const onGame=location.host.includes("games.shopee.vn");if(!onGame)await ensureGameOpen();})();
})();
