...
function log(...a){
  const box=q("#sf_log"); if(!box) return;
...
async function autowater(){
  if(state.running) return log("Tác vụ khác đang chạy.");
  state.running=true;state.what="autowater";
  ...
  try{
    ...
    while(state.running){
      ...
      if(!crop) break;
      const info=levelInfo(crop);
      if(info.tot>=info.max){
        log("Đã max EXP."); break;
      }
    ...
    state.running=false;state.what=null;
